...
import { allProducts } from "@/data/products-data"; // Import dữ liệu sản phẩm
...
export function ProductListing() {
  ...
  const productTypes = [
    ...
    { id: "tra-trai-cay", name: "Trà trái cây", count: allProducts.filter(p => p.type === "tra-trai-cay").length },
  ];

  const badges = [
    { id: "do-an-thuan-thuc-vat", name: "Đồ ăn thuần thực vật", count: allProducts.filter(p => p.badges.includes("do-an-thuan-thuc-vat")).length },
    { id: "san-pham-lon", name: "Sản phẩm lon", count: allProducts.filter(p => p.badges.includes("san-pham-lon")).length },
    { id: "tra-trai-cay-badge", name: "Trà trái cây", count: allProducts.filter(p => p.badges.includes("tra-trai-cay-badge")).length },
  ];
        ...
        <div className="lg:col-span-3">
          <div className="mb-6">
            <h3 className="text-xl font-semibold">
              Đã tìm thấy <span className="text-primary">{filteredProducts.length}</span> sản phẩm
            </h3>
          </div>
          {filteredProducts.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center p-12 text-center text-muted-foreground">
              ...
              <p className="text-sm mt-2">Hãy thử điều chỉnh lại bộ lọc của bạn.</p>
              <Button onClick={clearAllFilters} className="mt-4">Xóa bộ lọc</Button>
            </div>
          )}
        </div>
      </div>
